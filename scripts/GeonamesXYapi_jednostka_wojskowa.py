...
# Zapytanie do GeoNames (globalnie, bez country='PL')
...
def update_coordinates():
    ...
    cursor.execute("SELECT ID, Miejscowosc, X, Y FROM jednostka_wojskowa ORDER BY ID")
    ...
    for row in rows:
        ...
        if lat and lon:
            cursor.execute("UPDATE jednostka_wojskowa SET X = ?, Y = ? WHERE ID = ?", (lat, lon, id_))
